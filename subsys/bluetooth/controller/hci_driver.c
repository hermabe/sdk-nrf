...
static int configure_memory_usage(void)
{
	...
	cfg.periodic_adv_rsp_tx_buffer_cfg.count =
		CONFIG_BT_CTLR_SDC_PERIODIC_ADV_RSP_TX_BUFFER_COUNT;
	required_memory = sdc_cfg_set(SDC_DEFAULT_RESOURCE_CFG_TAG,
				      SDC_CFG_TYPE_PERIODIC_ADV_RSP_TX_BUFFER_CFG, &cfg);
	if (required_memory < 0) {
		return required_memory;
	}

	cfg.periodic_adv_rsp_max_tx_data_size =
		CONFIG_BT_CTLR_SDC_PERIODIC_ADV_RSP_TX_MAX_DATA_SIZE;
	required_memory = sdc_cfg_set(SDC_DEFAULT_RESOURCE_CFG_TAG,
				      SDC_CFG_TYPE_PERIODIC_ADV_RSP_MAX_TX_DATA_SIZE, &cfg);
	if (required_memory < 0) {
		return required_memory;
	}

	cfg.periodic_adv_rsp_rx_buffer_cfg.count =
		CONFIG_BT_CTLR_SDC_PERIODIC_ADV_RSP_RX_BUFFER_COUNT;
	required_memory = sdc_cfg_set(SDC_DEFAULT_RESOURCE_CFG_TAG,
				      SDC_CFG_TYPE_PERIODIC_ADV_RSP_RX_BUFFER_CFG, &cfg);
	if (required_memory < 0) {
		return required_memory;
	}
#endif
